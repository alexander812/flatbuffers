...
export class Resource implements flatbuffers.IUnpackableObject<ResourceT> {
  ...
  static createResource(
    ...
    Resource.addType(builder, typeOffset);
    return Resource.endResource(builder);
  }

  unpack(): ResourceT {
    return new ResourceT(this.bb!.createScalarList<number>(this.data.bind(this), this.dataLength()), this.type());
  }

  unpackTo(_o: ResourceT): void {
    _o.data = this.bb!.createScalarList<number>(this.data.bind(this), this.dataLength());
    _o.type = this.type();
  }
}

export class ResourceT implements flatbuffers.IGeneratedObject {
  constructor(public data: number[] = [], public type: string | Uint8Array | null = null) {}

  pack(builder: flatbuffers.Builder): flatbuffers.Offset {
    const data = Resource.createDataVector(builder, this.data);
    const type = this.type !== null ? builder.createString(this.type) : 0;

    return Resource.createResource(builder, data, type);
  }
}
