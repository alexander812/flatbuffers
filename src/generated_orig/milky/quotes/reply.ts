...
import type { ErrorT } from '../../milky/quotes/error.js';
import { Error } from '../../milky/quotes/error.js';
import { QuotesReplys, unionToQuotesReplys, unionListToQuotesReplys } from '../../milky/quotes/quotes-replys.js';
...
export class Reply implements flatbuffers.IUnpackableObject<ReplyT> {
  ...
  replyType(): QuotesReplys {
    const offset = this.bb!.__offset(this.bb_pos, 4);
    return offset ? this.bb!.readUint8(this.bb_pos + offset) : QuotesReplys.NONE;
  }
  ...
  static addReplyType(builder: flatbuffers.Builder, replyType: QuotesReplys) {
    builder.addFieldInt8(0, replyType, QuotesReplys.NONE);
  }
  ...
  unpack(): ReplyT {
    return new ReplyT(
      this.replyType(),
      (() => {
        const temp = unionToQuotesReplys(this.replyType(), this.reply.bind(this));
        if (temp === null) {
          return null;
        }
        return temp.unpack();
...
export class ReplyT implements flatbuffers.IGeneratedObject {
  constructor(public replyType: QuotesReplys = QuotesReplys.NONE, public reply: ErrorT | OkT | null = null) {}

  pack(builder: flatbuffers.Builder): flatbuffers.Offset {
    const reply = builder.createObjectOffset(this.reply);

    return Reply.createReply(builder, this.replyType, reply);
  }
}
