...
import { Params, unionToParams, unionListToParams } from '../../milky/config/params';
...
unpack(): RequestT {
  return new RequestT(
    this.bb!.createScalarList(this.paramsType.bind(this), this.paramsTypeLength()),
    (() => {
    let ret = [];
    for(let targetEnumIndex = 0; targetEnumIndex < this.paramsTypeLength(); ++targetEnumIndex) {
      let targetEnum = this.paramsType(targetEnumIndex);
      if(targetEnum === null || Params[targetEnum!] === 'NONE') { continue; }

      let temp = unionListToParams(targetEnum, this.params.bind(this), targetEnumIndex);
...
unpackTo(_o: RequestT): void {
  _o.paramsType = this.bb!.createScalarList(this.paramsType.bind(this), this.paramsTypeLength());
  _o.params = (() => {
    let ret = [];
    for(let targetEnumIndex = 0; targetEnumIndex < this.paramsTypeLength(); ++targetEnumIndex) {
      let targetEnum = this.paramsType(targetEnumIndex);
